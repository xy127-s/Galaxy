...
# 1. 页面基础配置
st.set_page_config(
    page_title="职教学生反诈识别系统",
    page_icon="🛡️",
    layout="centered"
)

# 2. 标题与介绍
st.title("🛡️ 职教学生专属反诈识别系统")
...
if st.button("🔍 一键检测诈骗风险", type="primary", use_container_width=True):
    if not input_text.strip():
        ...
        # 风险等级标签
        if result["风险等级"] == "高风险":
            st.error(f"⚠️ 风险等级：{result['风险等级']}")
        elif result["风险等级"] == "中风险":
            st.warning(f"⚠️ 风险等级：{result['风险等级']}")
        else:
            st.success(f"✅ 风险等级：{result['风险等级']}")
        
        # 诈骗类型与处置建议
        st.info(f"📌 诈骗类型：{result['诈骗类型']}")
        st.markdown(f"💡 处置建议：{result['处置建议']}")